...
void CudaContext::finalizeCalculation(std::vector<double*>& output) {
    ...
    if (!hasKernel_[currentId_ - 1]) {
        ...
        //std::string kernelSourceFromFile = loadKernelFromFile("cudaKernelProcessed.txt");
        //std::cout << kernelSourceFromFile << std::endl;
        // Compile source code
        nvrtcProgram nvrtcProgram;
        ...
        //nvrtcCall(nvrtcCreateProgram(&nvrtcProgram, kernelSourceFromFile.c_str(), "kernel.cu", 0, nullptr, nullptr), "finalizeCalculation(), nvrtcCreateProgram()");
